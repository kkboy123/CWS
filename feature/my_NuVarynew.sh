...
$svmscale -s scale top1.table > top1.table.scale 2> /dev/null;
$svmscale -r scale testdata.table > testdata.table.scale 2> /dev/null;
echo '--scaling 完成--'
echo '--one-class機器學習--';
$svmtrain -n $1 -s 2 top1.table.scale;
echo 'one-class機器學習訓練完成';
echo '開始進行predict';
$svmpredict testdata.table.scale top1.table.scale.model testdata.table.scale.predict;
...
wc -l machine.sl top1.all >> run.log;
echo '製作match.sl　檔案中..';
$getmatch machine.sl top1.all > match.sl;
...
echo 將訓練資料合併;
cat top1.table negative.table  > forBinarytraindata.table;
...
$getmatch machine.2.sl top1.all > match.2.sl;
